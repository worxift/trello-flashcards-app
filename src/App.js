...
const List = ({ list, onCardClick, selectedCardId, isArchive, handleDragStart, handleDrop, dragOverList, setDragOverList }) => {
    return (
        <div 
            className={`bg-gray-800 rounded-lg p-2 w-64 h-[calc(100vh-110px)] flex flex-col flex-shrink-0 transition-colors ${dragOverList === list.id ? 'bg-gray-900' : ''}`}
        ...
        >
            ...
            <div className={`flex-grow min-h-[100px] space-y-2 p-1 rounded-md overflow-y-auto pr-2 hover-scrollbar`} style={{ maxHeight: 'calc(100vh - 170px)', scrollbarWidth: 'thin' }}>
                {list.cards.map((card) => (
    ...
    React.useEffect(() => {
        const handleKeyDown = (event) => {
            ...
            setBoards(prevBoards => {
                ...
                // 智能刷词逻辑：在任何列表中按左键时，当前卡片标记为困难，上方卡片标记为容易
                if (event.key === 'ArrowLeft') {
                    ...
                    if (cardIndex > -1) {
                        ...
                        let hardListIndex, easyListIndex;
                        
                        // 确定目标列表索引
                        if (sourceListTitle === 'Inbox') {
                            // Inbox：左键->10，上方卡片->9
                            hardListIndex = 1; // 清单10
                            easyListIndex = 2; // 清单9
                        } else if (sourceListTitle === '10') {
                            // 列表10：左键->Inbox，上方卡片->9
                            hardListIndex = 0; // Inbox
                            easyListIndex = 2; // 清单9
                        } else if (sourceListTitle === 'Archive') {
                            // Archive不需要特殊处理，使用普通逻辑
                            targetListIndex = currentListIndex - 1;
                            return prevBoards; // 使用常规逻辑
                        } else {
                            // 获取当前列表的数字索引（从标题推断）
                            const currentNum = parseInt(sourceListTitle);
                            if (!isNaN(currentNum)) {
                                // 对于1-9的列表，左键移到左侧列表(当前数字+1)，上方卡片移到右侧列表(当前数字-1)
                                hardListIndex = board.lists.findIndex(l => l.title === String(currentNum + 1));
                                easyListIndex = board.lists.findIndex(l => l.title === String(currentNum - 1));
                                
                                // 对于列表1的特殊处理，右侧是Archive
                                if (sourceListTitle === '1') {
                                    easyListIndex = board.lists.findIndex(l => l.title === 'Archive');
                                }
                            } else {
                                // 如果不是数字标题，使用常规逻辑
                                targetListIndex = currentListIndex - 1;
                        ...
                        // 确保找到了有效的列表索引
                        if (hardListIndex === -1 || easyListIndex === -1) {
                            // 找不到对应列表，回退到常规逻辑
                            targetListIndex = currentListIndex - 1;
                            return prevBoards;
                        }
                        
                        const hardList = board.lists[hardListIndex];
                        const easyList = board.lists[easyListIndex];
        ...
        <div className="bg-gray-900 text-white h-screen flex flex-col font-sans">
            ...
            <div className="flex flex-grow overflow-hidden">
                ...
                <main className="flex-grow flex flex-col overflow-hidden">
                    {activeBoard ? (
                        <div className="flex-grow p-0 flex space-x-4 overflow-x-auto overflow-y-hidden">
